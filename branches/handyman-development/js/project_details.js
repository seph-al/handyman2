$(document).ready(function(){
		...
		$('#post_project').click(function(){
			...
			if(indicator == "2"){
				...
				else if(howeown_pname.length < 3 || howeown_pname.length > 12){
					$("#errors2").html('<div class="alert alert-danger">Public name must be 3 to 12 characters. <button type="button" class="close" data-dismiss="alert">&times;</button></div>');
					$("#howeown_pname").focus();
			...
			if(projecttype == "" || projecttype == "Please Select" ){
				...
				$.ajax({
			        ...
			        success: function(response){
			        	if (response.status){
			   				if (response.indicator==1 || response.indicator==3){
