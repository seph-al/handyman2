...
class Eventlog extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('SiteId, Created', 'required'),
			array('SiteId, Type', 'numerical', 'integerOnly'=>true),
			array('Description', 'length', 'max'=>1000),
			array('Title', 'length', 'max'=>100),
			array('IpAddress', 'length', 'max'=>55),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('LogId, SiteId, Description, Title, IpAddress, Type, Created', 'safe', 'on'=>'search'),
		);
	}
